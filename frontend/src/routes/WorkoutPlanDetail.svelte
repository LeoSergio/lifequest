<script>
  ...
  import { MUSCLE_GROUPS, EQUIPMENT_TYPES, WEEKDAYS } from '../lib/constants.js';
  import {
    workoutPlansQuery,
  ...
  $: if (currentPlan && (isEditing || isNew)) {
     if (!hasInitializedEdit) {
       editName = currentPlan.name || '';
       try { editWeekdays = JSON.parse(currentPlan.weekday || '[]'); } catch { editWeekdays = currentPlan.weekday ? [currentPlan.weekday] : []; }
  ...
  function toggleEditDay(val) {
    if (editWeekdays.includes(val)) {
      editWeekdays = editWeekdays.filter(d => d !== val);
    } else {
      editWeekdays = [...editWeekdays, val];
    }
  }

  async function handleUpdatePlan() {
    if (editName.trim()) {
      await updatePlan(planId, { name: editName.trim(), weekday: JSON.stringify(editWeekdays) });
    }
  }
...
<main class="min-h-screen p-4 pb-24 max-w-md mx-auto">
  ...
  <div class="flex justify-between items-center mb-6">
    {#if currentPlan}
      {#if isEditing || isNew}
        <input class="bg-white/5 border border-white/10 rounded-[10px] px-3 py-2 text-[16px] font-bold text-white focus:border-[#a855f7] outline-none placeholder:text-white/30 flex-1 mr-4" bind:value={editName} />
      {:else}
        <h1 class="text-2xl font-black text-white">{currentPlan.name}</h1>
      {/if}
    {/if}

    {#if !activeSession && (isEditing || isNew)}
      <button 
        class="text-[9px] uppercase font-bold tracking-wider px-3 py-2 rounded-[8px] transition-colors border bg-[#9333EA] border-[#9333EA] text-white shadow-[0_0_10px_rgba(147,51,234,0.3)] shrink-0" 
        on:click={async () => { await handleUpdatePlan(); isEditing = false; isNew = false; }}
      >
        Concluir Edição
      </button>
    {:else if !activeSession}
      <button 
        class="text-[9px] uppercase font-bold tracking-wider px-3 py-2 rounded-[8px] transition-colors border border-white/10 text-white/50 hover:text-white hover:bg-white/5 shrink-0 flex items-center gap-1" 
  ...
  {#if !activeSession && (isEditing || isNew)}
    <div class="bg-[#1C1C22]/80 border border-white/5 rounded-[20px] p-5 mb-6 flex flex-col gap-3 shadow-inner">
      <label class="text-[11px] uppercase text-white/40 mb-1 font-bold tracking-wider flex items-center gap-2">Dias da Semana</label>
      <div class="flex flex-wrap gap-2">
        {#each activeDays as day}
          <button
            type="button"
            class="px-3 py-2 text-[10px] font-bold uppercase tracking-wider rounded-[10px] border transition-all {editWeekdays.includes(day.value) ? 'bg-[#9333EA] border-[#9333EA] text-white shadow-[0_0_10px_rgba(147,51,234,0.3)]' : 'bg-white/5 border-white/10 text-white/40 hover:text-white/70 hover:bg-white/10'}"
        ...
        <button
          type="button"
          class="px-3 py-2 text-[10px] font-bold uppercase tracking-wider rounded-[10px] border transition-all {editWeekdays.length === 0 ? 'bg-white/10 border-white/20 text-white' : 'bg-white/5 border-white/10 text-white/40 hover:text-white/70 hover:bg-white/10'}"
          on:click={() => editWeekdays = []}
        >
          Livre
        </button>
      </div>
  ...
  {:else}
    {#if exercises.length === 0 && !isEditing && !isNew}
      <div class="bg-[#1C1C22]/80 border border-white/5 rounded-[20px] p-6 text-center shadow-inner mb-6">
         ...
         <p class="text-[10px] text-white/40 mb-3">Ficha vazia. Adicione exercícios para começar.</p>
         <button class="bg-[#9333EA] text-white rounded-[12px] px-4 py-2 text-[11px] font-bold shadow-[0_0_15px_rgba(147,51,234,0.3)] hover:bg-[#a855f7] transition-colors" on:click={() => isEditing = true}>
    ...
    {#if todaysFinishedSession && !isEditing && !isNew}
